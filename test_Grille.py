...
def test_verifier_tuile_vide():
    ...
    grille.ajouter_tuile(2, 0, 1)  # Ajoute une tuile de type 1 à la position (0, 1)
    ...
    assert (
        grille.verifier_tuile_vide() == False
    )  # Les coordonnées (1, 0) et (2, 0) sont vides et forment un trimino valide
...
def test_verifier_tableau():
    ...
    assert (
        grille.verifier_tableau() == True
    )  # La grille est vide, donc il y a de la place pour un trimino
    ...
    assert (
        grille.verifier_tableau() == True
    )  # Il y a encore de la place pour un trimino
